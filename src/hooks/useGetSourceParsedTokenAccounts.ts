...
const getSolanaParsedTokenAccounts = async (
  ...
  try {
    ...
    if (nft) {
      ...
      const USDC_Token = process.env.REACT_APP_Solana_USDC_Token_Address as string;

      let splParsedTokenAccount = splParsedTokenAccounts.find(e => e.mintKey === USDC_Token);
      dispatch(receiveSourceParsedTokenAccounts(splParsedTokenAccounts));
      dispatch(setSourceParsedTokenAccount(splParsedTokenAccount))
      // dispatch(setSourceWalletAddress(walletAddress));

      console.log('splParsedTokenAccount')
      console.log(splParsedTokenAccount)
    }
  } catch (e) {
    console.error(e);
...
function useGetAvailableTokens(nft: boolean = false) {
  ...
  const fetchEthereumUSDC = useCallback( async() => {
    let USDK_Token = process.env.REACT_APP_Ethereum_USDK_Token_Address as string;
    if (lookupChain == CHAIN_ID_POLYGON) {
      USDK_Token = process.env.REACT_APP_Polygon_USDK_Token_Address as string;
    }

    const tokenAccount = await getEthereumToken(USDK_Token, provider as Web3Provider);
    const parsedTokenAccount = await ethTokenToParsedTokenAccount(tokenAccount as TokenImplementation, signerAddress as string);
    dispatch(setSourceParsedTokenAccount(parsedTokenAccount))
  ...
  }, [lookupChain, provider, signerAddress, dispatch, nft, covalent, fetchEthereumUSDC]);
